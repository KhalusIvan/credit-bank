        ...
        <div className={`credit-item ${props.status}`} >
            ...
            <div className='credit-body d-flex justify-content-between'>
                ...
                <div className='credit-payd-data'>
                    ...
                    <button   data-toggle='modal' data-target={'#'+props.idOfModal} onClick={()=>{props.getDataAboutCredit(props.id)}} className={`btn w-100 btn-dark ${props.status === 'closed' ? 'd-none':''}`}>{appLanguage === 'eng' ? 'Pay' : 'Оплатити'}</button>
                </div>
            </div>
