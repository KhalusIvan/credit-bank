                    ...
                    <ul className="navbar-nav navbar-links ml-auto">
                        {user.role === 'guest' ?
                            ...
                            (<>
                                ...
                                <li className={`nav-item ${path === '/user/logOut' ? 'active' : ''}`}><Link className={`nav-link ${user.email?'':'disabled'}`} onClick={() => { localStorage.removeItem('adminToken'); changeUserRole('guest');changeParam('') }} to={`/guest`}>{language.appLanguage === 'eng' ? 'Log out' : 'Вийти'}</Link></li>
                            </>) 
                        }
                    </ul>
                </div>
